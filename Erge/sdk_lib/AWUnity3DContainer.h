 ...
 *
 * function awAdNotifyToFetchBonus(score:string, adType:string)——当用户观看完视频激励广告并获得奖励之后，SDK向当前目标发送此消息。score参数为用户获得多少积分。adType为AWU3D_AD_TYPE中的值
 *
 ...
 *
 * function awUserClosedVideoAd(closeType:string, adType:string)——当当前的视频被用户关闭时，SDK会给当前目标发送此消息。closeType为"0"，说明用户至少已经完整地看过一次视频；closeType为"1"时，说明用户在第一次看视频尚未看完就关闭了。adType为AWU3D_AD_TYPE中的值
 *
